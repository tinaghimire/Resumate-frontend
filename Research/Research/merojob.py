...
def get_job_links(category_link):
    ...
    for index, job_element in enumerate(job_elements):
        ...
        if link_element:
            ...
            # Extracts title
            job_title=job_link_soup.find('h1',class_='h4 mb-0 text-primary').text.strip()
            ...
            with open(f'JD/{job_title}.txt','w') as f:
...
for category_element in category_elements:
    ...
    if link_element:
        ...
        if user_input.lower() in category_name.lower():
            ...
            page_no_list = category_soup.find('ul', class_='pagination pagination-sm')
            if page_no_list:
                li_list = page_no_list.find_all('li', class_='page-item')
                i=0
                for pages in li_list:  
                    #print(li_list)
                    if i == len(li_list)-1:
                        break
                    #print('THIS IS PAGES',pages)
                    next_link = pages.find('a')
                    #print({next_link})
                    next_page_link = next_link['href'] 
                    category_link = urljoin(category_link,next_page_link)    
                    #print('THIS IS CATEGORY LINK',{category_link})
                    job_data_list = get_job_links(category_link) 
                    i = i+1           
    else:
